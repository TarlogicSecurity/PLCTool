...
bool
CGXDLMSTranslator::GetComments()
{
  return m_Comments;
}

void
CGXDLMSTranslator::SetComments(bool value)
{
  m_Comments = value;
}

bool
CGXDLMSTranslator::GetPduOnly()
{
  return m_PduOnly;
}

void
CGXDLMSTranslator::SetPduOnly(bool value)
{
  m_PduOnly = value;
}

bool
CGXDLMSTranslator::GetCompletePdu()
{
  return m_CompletePdu;
}

void
CGXDLMSTranslator::SetCompletePdu(bool value)
{
  m_CompletePdu = value;
}

bool
CGXDLMSTranslator::GetHex()
{
  return m_Hex;
}

void
CGXDLMSTranslator::SetHex(bool value)
{
  m_Hex = value;
}

bool
CGXDLMSTranslator::GetShowStringAsHex()
{
  return m_ShowStringAsHex;
}

void
CGXDLMSTranslator::SetShowStringAsHex(bool value)
{
  m_ShowStringAsHex = value;
}

bool
CGXDLMSTranslator::GetOmitXmlDeclaration()
{
  return m_OmitXmlDeclaration;
}

void
CGXDLMSTranslator::SetOmitXmlDeclaration(bool value)
{
  m_OmitXmlDeclaration = value;
}

bool
CGXDLMSTranslator::GetOmitXmlNameSpace()
{
  return m_OmitXmlNameSpace;
}

void
CGXDLMSTranslator::SetOmitXmlNameSpace(bool value)
{
  m_OmitXmlNameSpace = value;
}

DLMS_SECURITY
CGXDLMSTranslator::GetSecurity()
{
  return m_Security;
}

void
CGXDLMSTranslator::SetSecurity(DLMS_SECURITY value)
{
  m_Security = value;
}

CGXByteBuffer &
CGXDLMSTranslator::GetSystemTitle()
{
  return m_SystemTitle;
}

void
CGXDLMSTranslator::SetSystemTitle(CGXByteBuffer &value)
{
  m_SystemTitle.Clear();
...
CGXByteBuffer &
CGXDLMSTranslator::GetBlockCipherKey()
{
  return m_BlockCipherKey;
}

void
CGXDLMSTranslator::SetBlockCipherKey(CGXByteBuffer &value)
{
...
CGXByteBuffer &
CGXDLMSTranslator::GetAuthenticationKey()
{
  return m_AuthenticationKey;
}

void
CGXDLMSTranslator::SetAuthenticationKey(CGXByteBuffer &value)
{
  m_AuthenticationKey.Clear();
  m_AuthenticationKey.Set(
      value.GetData(),
      value.GetSize() - value.GetPosition());
}

unsigned long
CGXDLMSTranslator::GetFrameCounter()
{
  return m_FrameCounter;
}

void
CGXDLMSTranslator::SetFrameCounter(unsigned long value)
{
  m_FrameCounter = value;
}

void
CGXDLMSTranslator::GetCiphering(CGXDLMSSettings &settings, bool force)
{
...
int
GetUa(CGXByteBuffer &data, CGXDLMSTranslatorStructure *xml)
{
    ...
    std::string &output)
{
  ...
  switch (cmd) {
    ...
    case DLMS_COMMAND_GET_REQUEST:
      CGXDLMSLNCommandHandler::HandleGetRequest(
          settings,
          NULL,
          value,
          NULL,
    ...
    case DLMS_COMMAND_SET_REQUEST:
      CGXDLMSLNCommandHandler::HandleSetRequest(
          settings,
          NULL,
          value,
          NULL,
    ...
    case DLMS_COMMAND_READ_REQUEST:
      CGXDLMSSNCommandHandler::HandleReadRequest(
          settings,
          NULL,
          value,
          NULL,
    ...
    case DLMS_COMMAND_METHOD_REQUEST:
      CGXDLMSLNCommandHandler::HandleMethodRequest(
          settings,
          NULL,
          value,
          NULL,
    ...
    case DLMS_COMMAND_WRITE_REQUEST:
      CGXDLMSSNCommandHandler::HandleWriteRequest(
          settings,
          NULL,
          value,
          NULL,
    ...
    case DLMS_COMMAND_ACCESS_REQUEST:
      CGXDLMSLNCommandHandler::HandleAccessRequest(
          settings,
          NULL,
          value,
          NULL,
    ...
    case DLMS_COMMAND_RELEASE_REQUEST:
      ...
      if (value.Available() != 0) {
        ...
        if (xml->GetOutputType() == DLMS_TRANSLATOR_OUTPUT_TYPE_SIMPLE_XML) {
          str = CTranslatorSimpleTags::ReleaseRequestReasonToString(
              (DLMS_RELEASE_REQUEST_REASON) ch);
        } else {
          str = CTranslatorStandardTags::ReleaseRequestReasonToString(
              (DLMS_RELEASE_REQUEST_REASON) ch);
        }
    ...
    case DLMS_COMMAND_RELEASE_RESPONSE:
      ...
      if (ch != 0) {
        ...
        if (xml->GetOutputType() == DLMS_TRANSLATOR_OUTPUT_TYPE_SIMPLE_XML) {
          str = CTranslatorSimpleTags::ReleaseResponseReasonToString(
              (DLMS_RELEASE_RESPONSE_REASON) ch);
        } else {
          str = CTranslatorStandardTags::ReleaseResponseReasonToString(
              (DLMS_RELEASE_RESPONSE_REASON) ch);
        }
        xml->AppendLine(DLMS_TRANSLATOR_TAGS_REASON, "Value", str);
        if (value.Available() != 0) {
    ...
    case DLMS_COMMAND_DED_METHOD_RESPONSE:
      if (settings.GetCipher() != NULL && m_Comments) {
        ...
        CGXByteBuffer *st;
        if (c == DLMS_COMMAND_GLO_READ_REQUEST ||
            c == DLMS_COMMAND_GLO_WRITE_REQUEST ||
            c == DLMS_COMMAND_GLO_GET_REQUEST ||
            c == DLMS_COMMAND_GLO_SET_REQUEST ||
            c == DLMS_COMMAND_GLO_METHOD_REQUEST ||
            c == DLMS_COMMAND_DED_GET_REQUEST ||
            c == DLMS_COMMAND_DED_SET_REQUEST ||
            c == DLMS_COMMAND_DED_METHOD_REQUEST) {
        ...
        if (ret != 0) {
          ...
          if (st.GetSize() != 0) {
            if (settings.GetCipher()->Decrypt(
                    ...
                    suite) == 0) {
              xml->StartComment(
                  "Decrypt data: " + data.GetData().ToHexString());
              ret = PduToXml(
                  xml,
                  data.GetData(),
                  omitDeclaration,
    ...
    default:
      ...
      value.SetPosition(value.GetPosition() - 1);
      str.append(
          value.ToHexString(value.GetPosition(), value.Available(), false));
      str.append("\" />");
      xml->AppendLine(str);
      break;
  }
...
DLMS_ERROR_CODE
CGXDLMSTranslator::ValueOfErrorCode(
...
int
CGXDLMSTranslator::DataToXml(CGXByteBuffer &data, std::string &xml)
{
  int ret;
  CGXDLMSVariant value;
  CGXDataInfo di;
  CGXDLMSSettings settings(false);
  CGXDLMSTranslatorStructure tmp(
      m_OutputType,
      m_OmitXmlNameSpace,
      m_Hex,
      m_ShowStringAsHex,
...
CGXByteBuffer &
CGXDLMSTranslator::GetServerSystemTitle()
{
  return m_ServerSystemTitle;
}

void
CGXDLMSTranslator::SetServerSystemTitle(CGXByteBuffer &value)
{
  m_ServerSystemTitle.Clear();
  m_ServerSystemTitle.Set(
      value.GetData(),
      value.GetSize() - value.GetPosition());
}

CGXByteBuffer &
CGXDLMSTranslator::GetDedicatedKey()
{
  return m_DedicatedKey;
}

void
CGXDLMSTranslator::SetDedicatedKey(CGXByteBuffer &value)
{
